...
const Navbar = ({ drawer, handleChange, isAuth, profile, login, logout }) => {
  return (
    <nav className={drawer ? s.wideNav : s.nav}>
        ...
        <li className={s.navigationItem}>
          {isAuth ? (
            <NavLink to='/profile' className={s.navigationLink} activeClassName={s.active}>
              <img
                className={s.navigationIcon}
                src={!profile || !profile.photos.small ? userPhoto : profile.photos.small}
                alt='avatar'
              />
